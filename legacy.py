"""
The code that's not needed anymore from the original project.

This fork might need these later on for enhancements.

Note: these are not in a working state atm.
"""
import argparse
import time
import traceback
...
def artist_match(tidal_track, spotify_track):
    ...
    # Try with both un-normalized and then normalized
    if (
        get_tidal_artists(tidal_track).intersection(get_spotify_artists(spotify_track))
        != set()
    ):
        return True
    return (
...
def from_main():
    ...
    # This was below "Finished execution" in main.

    if args.uri:
        # if a Spotify playlist ID is explicitly provided
        # as a command line argument then use that.
